...
class SetpointTrajectoryGenerator:
    ...
    def update(
        ...
        if distance_xy <= self.arrival_position_tol and speed_xy <= self.arrival_velocity_tol:
            px, py = tx, ty
            vx, vy = 0.0, 0.0
        else:
            brake_speed = max(
            ...
            if (
                math.hypot(tx - px, ty - py) <= self.arrival_position_tol
                and math.hypot(vx, vy) <= self.arrival_velocity_tol
            ):
                px, py = tx, ty
                vx, vy = 0.0, 0.0

        error_z = tz - pz
        if abs(error_z) <= self.arrival_position_tol and abs(vz) <= self.arrival_velocity_tol:
            pz = tz
            vz = 0.0
        else:
...
class MyLinkMavlinkClient:
    ...
    def _send_setpoint(self) -> None:
        ...
        with self._trajectory_lock:
            ...
            generated = self._trajectory.update(actual_position, dt)
...
class MyLinkController:
    ...
    def takeoff(self, altitude: str | float) -> None:
        ...
        self._log_target_change("TAKEOFF", old, new)
